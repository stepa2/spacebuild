...
function ENT:DoNormalDraw(bDontDrawModel)
	...
	local entid = self:GetNWInt("entid")
	...
	if not mode or mode ~= 2 then
		...
		if entid == 0 then
			OverlayText = OverlayText .. "Not connected to an Entity\n"
		else
			OverlayText = OverlayText .. "Connected to Entity " .. tostring(entid) .. "\n"
		end
	...
	if entid == 0 then
		surface.DrawText("Not connected to an Entity")
	else
		surface.DrawText("Connected to Entity " .. entid)
	end
