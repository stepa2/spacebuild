include("shared.lua")
language.Add("caf_rd_other_screen", "Life Support Screen")
local MainFrames = {}
...
function ENT:DoNormalDraw(bDontDrawModel)
	...
	if string.find(mdl, "s_small_screen") then
		mul_ri = -8.25
		mul_fr = -6.25
		res = 0.025
		mul = 0.5
	elseif string.find(mdl, "small_screen") then
		mul_ri = -16.5
		mul_fr = -12.5
		res = 0.05
	elseif string.find(mdl, "medium_screen") then
		mul_ri = -33
		mul_fr = -25
		res = 0.1
		mul = 1.5
	elseif string.find(mdl, "large_screen") then
		mul_ri = -66
		mul_fr = -50
		res = 0.2
		mul = 2
	...
	cam.Start3D2D(pos, self:GetAngles(), res)
